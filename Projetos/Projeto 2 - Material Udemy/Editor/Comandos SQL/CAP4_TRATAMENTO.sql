...
--
SELECT Concat(COD_ALUNO,NOME) FROM TALUNO2;

--
SELECT COD_ALUNO||' - '||NOME FROM TALUNO2;

--
SELECT nome, InitCap(NOME) FROM TALUNO2;

--
SELECT nome, InStr(NOME,'R') FROM TALUNO2;

--
SELECT nome, Length(NOME) FROM TALUNO2;

--
SELECT nome, Lower(NOME) FROM TALUNO2

--
SELECT nome, Upper(NOME) FROM TALUNO2;

--
SELECT InitCap('JOSE DA SILVA') FROM dual;

-- left - esquerda
SELECT cod_aluno, LPad(COD_ALUNO,5,'0') FROM TALUNO2;

-- direita
SELECT nome, salario, RPad(SALARIO,8,'0') FROM TALUNO2;

--
SELECT nome, RPad(NOME,10,'$') FROM TALUNO2;

-- copia parte de um texto
-- substr ( campo/texto, posicao, qtde de caract )
SELECT nome, SubStr(NOME,1,3) FROM TALUNO2;

--
SELECT SubStr(NOME,1,1) FROM TALUNO2;

--
SELECT nome, SubStr(NOME,3,1) FROM TALUNO2;

--
SELECT REPLACE(Upper(NOME),'R','$') FROM TALUNO2;

--

SELECT SubStr(NOME,Length(nome),1) FROM TALUNO2;

--
SELECT SubStr(NOME,Length(nome)-1, 2) FROM TALUNO2;

--
SELECT nome, SubStr(NOME, 3, Length(nome)-3 ) FROM TALUNO2;
...
SELECT
  SALARIO,
  REPLACE(SALARIO, ',' , '') AS "SALARIO SEM VÍRGULA", 
...
SELECT DATA, SYSDATE, DATA + 5 FROM TCONTRATO2;

SELECT SYSDATE - DATA AS DIF_DIAS FROM TCONTRATO2;

SELECT Trunc(SYSDATE - DATA) as DIAS FROM TCONTRATO2;

--Somando horas em uma data
SELECT SYSDATE, SYSDATE + 5 / 24 as ADD_HORAS FROM TCONTRATO2;

--Somar minutos
SELECT SYSDATE, SYSDATE + 15 / 1440 as ADD_MINUTOS FROM TCONTRATO2;

SELECT SYSDATE, SYSDATE + 30 / (3600 * 24) as ADD_SEGUNDOS FROM TCONTRATO2;
...
--Ultimo dia do mes
SELECT Last_Day(TRUNC(SYSDATE)) AS ULTIMO_DIA_MES FROM DUAL;
...
--
SELECT 'R$ '||(Round(Salario,2)) AS salario FROM TALUNO2;
...
SELECT Cod_Aluno, Nvl(Nome, 'SEM NOME') FROM TALUNO2
...
SELECT NOME, Estado,
       CASE
         ...
         WHEN CIDADE = 'BRASILIA' THEN 'BRASILIENSE'
