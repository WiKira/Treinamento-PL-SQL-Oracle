...
SELECT NOME AS CURSO,
       VALOR,
       VALOR/CARGA_HORARIA,                       --FUTURAMENTE PRA EVITAR ERRO COLOCAR UM COMANDO CASE AQUI
       ROUND(VALOR/CARGA_HORARIA,2) AS VALOR_HORA -- E AQUI
  FROM TCURSO
 ORDER BY VALOR_HORA;
...
--
SELECT COD_ALUNO || ' - ' || NOME || ' // ' || CIDADE AS ALUNO --UMA SUGESTÃO PRA EVITAR PROBLEMAS DE PERFORMANCE, 
  FROM TALUNO                                                  --UTILIZAR O SUBSTRING QUANDO CONCATENAR TEXTO MESMO QUE SEJA PARA O TAMANHO MÁXIMO DO CAMPO 
 ORDER BY COD_ALUNO;
