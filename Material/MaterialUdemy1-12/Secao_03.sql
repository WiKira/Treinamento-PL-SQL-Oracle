...
--
UPDATE TALUNO 
   SET NASCIMENTO='10/10/2001'
 WHERE COD_ALUNO=1;

--
UPDATE TALUNO 
   SET NASCIMENTO='10/08/2000'
 WHERE COD_ALUNO=2;
...
SELECT COD_ALUNO, 
       NASCIMENTO,
       TRUNC(NASCIMENTO) AS nascimento, --O comando AS é correto, mas não é necessário obrigatóriamente, você pode só colocar o nome com aspas dupla na frente da coluna
       NOME
  FROM TALUNO
 WHERE TRUNC(NASCIMENTO) = TO_DATE('25/08/2010', 'DD/MM/YYYY'); --QUANDO FOR COMPARAR DATA E TIVER ELA SÓ TIPO STRING, MELHOR CONVERTER ANTES DE COMPARAR, POR CONTA DA FORMATAÇÃO
...
SELECT * 
 ...
 WHERE DESCONTO >= 0
   AND DESCONTO <= 10
    OR DESCONTO IS NULL; --EU GERAMENTE QUANDO USO OR PROCURO USAR () PRA DETERMINAR CERTINHO ONDE QUERO AS CONDIÇÕES PQ POR EXEMPLO, PODE TER A INTERPRETAÇÃO
                         --QUE VAI SER O (DESCONTO MAIOR OU IGUAL A ZERO E DESCONTO MENOR OU IGUAL A 10) OU DESCONTO É NULO
                         --MAS TB PODE SER DESCONTO MAIOR OU IGUAL A ZERO E (DESCONTO MENOR OU IGUAL A 10 OU DESCONTO É NULO)
                         --COM AS () VC DEFINE MELHOR EXATAMENTE O QUE QUER E FACILITA O ENTENDIMENTO PRA QUEM ESTIVER LENDO
...
SELECT * 
  FROM TCURSO
 WHERE NOME LIKE 'W%'     --Registros que inicia com W
